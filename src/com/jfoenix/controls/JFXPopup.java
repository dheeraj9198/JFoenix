...
public class JFXPopup extends StackPane {
    ...
    public void showCustom(PopupVPosition vAlign, PopupHPosition hAlign, double initOffsetX, double initOffsetY) {
        offsetX = 0;
        offsetY = 0;
...
*/
        contentHolder.setTranslateX(initOffsetX - content.getWidth());

        double y =   initOffsetY - getHeight();

        if(bound.getMinY() > y){
            y = bound.getMinY();
        }else if(bound.getMaxX() < y){
            y = bound.getMaxY();
        }


        contentHolder.setTranslateY(y);
